...
# Run from anywhere: ./start.sh   or   bash /path/to/backend/scripts/start.sh
...
compose --env-file "$BACKEND_ROOT/services/auth/.env" -f "$BACKEND_ROOT/services/auth/deployments/docker-compose.yaml" up -d --build

echo "==> Service: users (8081)"
compose --env-file "$BACKEND_ROOT/services/users/.env" -f "$BACKEND_ROOT/services/users/deployments/docker-compose.yaml" up -d --build

echo "==> Service: content (8083)"
compose --env-file "$BACKEND_ROOT/services/content/.env" -f "$BACKEND_ROOT/services/content/deployments/docker-compose.yaml" up -d --build
...
echo "Auth/users/content use services/<svc>/.env for compose vars and container env_file."
