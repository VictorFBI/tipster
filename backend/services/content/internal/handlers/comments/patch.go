...
func PatchContentComments(w http.ResponseWriter, r *http.Request) {
	...
	comment, err := svc.UpdateComment(r.Context(), authorID, req.CommentId.String(), contentPtr, req.ImageObjectIds, auth)
	if err != nil {
		...
		if errors.Is(err, commentsservice.ErrNoUpdateFields) {
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusBadRequest)
			json.NewEncoder(w).Encode(api.ErrorResponse{Message: "send at least one of content or image_object_ids"})
			return
