...
import (
	...
	kafkadb "tipster/backend/content/internal/db/kafka"
	...
	registrationconfirmationservice "tipster/backend/content/internal/services/registrationconfirmation"
...
func ConfirmEmailRegistration(w http.ResponseWriter, r *http.Request) {
	...
	err = kafkaClient.NewWriter("content.user.created").WriteMessages(r.Context(), kafka.Message{
		Key: []byte(user.Id),
	})
