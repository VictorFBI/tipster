...
func GetContentPostsLiked(w http.ResponseWriter, r *http.Request) {
	log := logging.LoggerFromContext(r.Context()).With(slog.String("handler", "get_content_posts_liked"))
	...
	rows, err := svc.ListPostsLikedByUser(r.Context(), userID, limit, offset)
	if err != nil {
		log.Error("list_posts_liked_failed", slog.String("error", err.Error()))
		w.WriteHeader(http.StatusInternalServerError)
		return
	}

	items := make([]api.LikedPostItem, 0, len(rows))
	for i := range rows {
		ap, convErr := helpers.PostFromService(&rows[i].Post)
		if convErr != nil {
			log.Error("list_posts_liked_response_failed", slog.String("error", convErr.Error()))
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		items = append(items, api.LikedPostItem{
			Post:    ap,
			LikedAt: rows[i].LikedAt,
		})
	}

	log.Info("get_content_posts_liked_ok", slog.Int("count", len(items)))
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(api.LikedPostsPage{
		Items:  items,
		Limit:  limit,
		Offset: offset,
	})
