...
func MapMediaCommitErr(err error) (status int, message string, ok bool) {
	...
	case errors.Is(err, media.ErrTempImageNotFound):
		return http.StatusNotFound, "Avatar object was not found in temporary storage", true
	...
	default:
		...
		if strings.HasPrefix(s, "media service request:") {
			return http.StatusBadGateway, "Media service is unreachable: " + strings.TrimPrefix(s, "media service request:"), true
		}
		if strings.HasPrefix(s, "media commit unauthorized:") {
			return http.StatusBadGateway, "Media service rejected the token: " + strings.TrimPrefix(s, "media commit unauthorized:"), true
		}
		if strings.HasPrefix(s, "media commit failed:") {
			return http.StatusBadGateway, "Media service error: " + strings.TrimPrefix(s, "media commit failed:"), true
		}
		if strings.HasPrefix(s, "media commit bad request:") {
			return http.StatusBadRequest, "Media service bad request: " + strings.TrimPrefix(s, "media commit bad request:"), true
		}
		return 0, "", false
	}
}
