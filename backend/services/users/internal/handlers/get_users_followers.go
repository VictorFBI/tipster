...
func GetUsersFollowers(w http.ResponseWriter, r *http.Request) {
	log := logging.LoggerFromContext(r.Context()).With(slog.String("handler", "get_users_followers"))
	...
	rows, total, err := svc.ListFollowers(r.Context(), accountID, limit, offset)
	if err != nil {
		if errors.Is(err, usersservice.ErrInvalidAccountID) {
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusBadRequest)
		...
		log.Error("list_followers_failed", slog.String("error", err.Error()))
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	log.Info("get_users_followers_ok", slog.Int("count", len(items)), slog.Int("total", total))
	w.Header().Set("Content-Type", "application/json")
